...
- API boundary debt has started moving down: `pkg/response` no longer imports `internal/domain`, `internal/capabilities/ai` no longer imports `internal/infra/http`, and `internal/capabilities/workflow` no longer imports `internal/infra/config`.
...
4. For `internal/capabilities/workflow`, continue with the remaining `queue`, `retry`, or `schedule` reverse imports after the `config` adapter split.
