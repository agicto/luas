...
|---|---|---|---|---|
| `core` | `api/internal/bootstrap`, `api/internal/infra` except packages explicitly classified as capabilities below, `api/pkg`, `web/src/components/ui`, `web/src/config`, `web/src/http`, `web/src/i18n`, `web/src/themes`, and the matching static foundations under `web-spa/src/app`, `components/ui`, `config`, `http`, `i18n`, and `styles` | Long-lived runtime and infrastructure. | Keep the API core and selected browser shell; remove the unselected browser shell rather than importing across them. | `make check`; `bash .agents/skills/luas-framework-review/scripts/check-api-boundaries.sh` |
...
| `console` | `web/src/app/(protected)/(console)`, `web/src/components/features/console`, and the static shell under `web-spa/src/routes/console*` plus `web-spa/src/components/layout/console-layout.tsx` | Replaceable scaffold workspace. The Next.js version is authenticated; the initial static shell remains unprotected until a browser gateway exists. | Rename or redesign in downstream mode; do not turn it into a fixed downstream workspace or fake SPA authorization in Luas. | Selected browser-shell type, lint, test, and build checks |
...
Formal static SPA copy uses the aligned English and Chinese i18next resources under
`web-spa/src/i18n/`. Route, feature, and layout components must not introduce a second
...
7. Choose the downstream browser shell. Use [`../web/docs/ADDING_FEATURE.md`](../web/docs/ADDING_FEATURE.md) for Next.js features or [`../web-spa/docs/ADDING_FEATURE.md`](../web-spa/docs/ADDING_FEATURE.md) for static SPA features; remove the unselected shell.
...
| Static SPA feature, route, shell, or deployment changed | `cd web-spa && pnpm type-check && pnpm lint && pnpm test -- --run && pnpm build` |
