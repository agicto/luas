...
| `optional starter` | `api/internal/modules/organization` and `permission` plus their browser workflows in `web/src/features/organization` and `permission`; API starters use `OPTIONAL_STARTERS`, Web features use `NEXT_PUBLIC_OPTIONAL_FEATURES`. | Starter-quality behavior kept outside default runtime assembly and browser execution until explicitly chosen. Dependencies are explicit: `permission` requires `organization`. | Select only what the product needs, keep API replicas/migration jobs and the matching Web build aligned, and remove the catalog, feature, routes, i18n, and owned contract surfaces together. | `cd api && make test`; `cd web && pnpm test -- --run`; `go run ./cmd/luas route:list`; `make governance` |
...
| API starter kept, removed, or added | `cd api && make test`; `python3 .agents/skills/luas-framework-review/scripts/check-starter-catalog.py`; permission changes also run `python3 .agents/skills/luas-framework-review/scripts/check-permission-boundary.py`; package boundary check |
