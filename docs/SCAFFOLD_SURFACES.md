...
| `optional starter` | `api/internal/modules/organization` plus its browser workflow in `web/src/features/organization`; API starters use `OPTIONAL_STARTERS`, Web features use `NEXT_PUBLIC_OPTIONAL_FEATURES`. | Starter-quality behavior kept outside default runtime assembly and browser execution until explicitly chosen. | Select only what the product needs, keep API replicas/migration jobs and the matching Web build aligned, and remove the catalog, feature, routes, i18n, and owned contract surfaces together. | `cd api && make test`; `cd web && pnpm test -- --run`; `go run ./cmd/luas route:list`; `make governance` |
