...
| `console` | `web/src/app/(protected)/(console)`, `web/src/components/features/console`, and the static shell under `admin/src/routes/console*` plus `admin/src/components/layout/console-layout.tsx` | Replaceable scaffold workspace. The Next.js version is authenticated; the initial Admin shell remains unprotected until its browser auth feature and system-operator authorization are assembled. | Rename or redesign in downstream mode; do not turn it into a fixed downstream workspace or fake browser authorization in Luas. | Selected browser-shell type, lint, test, and build checks |
