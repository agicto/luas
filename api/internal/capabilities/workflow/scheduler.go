...
// ScheduledTask represents a schedulable workflow task.
type ScheduledTask interface {
	Run(ctx context.Context) error
}

// ScheduledTaskFunc adapts a function into a schedulable workflow task.
type ScheduledTaskFunc func(ctx context.Context) error

func (f ScheduledTaskFunc) Run(ctx context.Context) error {
	return f(ctx)
}

// Event represents a scheduled workflow event.
type Event struct {
	name            string
	task            ScheduledTask
	expression      string
...
// NewEvent creates a scheduled workflow event.
func NewEvent(name string, task ScheduledTask) *Event {
	return &Event{
		name:       name,
		task:       task,
		timezone:   time.Local,
...
// Call creates an event from a function.
func Call(name string, fn func(ctx context.Context) error) *Event {
	return NewEvent(name, ScheduledTaskFunc(fn))
}
...
// Scheduler manages scheduled workflow events.
...
// GlobalScheduler returns the shared scheduler.
...
func (s *Scheduler) Run(ctx context.Context) {
	...
	for _, event := range s.DueEvents(now) {
		if err := event.Run(ctx); err != nil {
			log.Printf("Scheduled task %q failed: %v", event.Name(), err)
		}
	}
}
...
// Schedule creates and registers a new event on the shared scheduler.
func Schedule(name string, fn func(ctx context.Context) error) *Event {
	return GlobalScheduler().Call(name, fn)
}

// RunSchedule runs due events on the shared scheduler.
func RunSchedule(ctx context.Context) {
	GlobalScheduler().Run(ctx)
}

// StartSchedule starts the shared scheduler.
func StartSchedule(ctx context.Context) {
	GlobalScheduler().Start(ctx)
}

// StopSchedule stops the shared scheduler.
func StopSchedule() {
