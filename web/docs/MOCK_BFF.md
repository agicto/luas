...
`src/app/api/**`. It lets the web shell run before a real backend is available. The auth route
handlers are hybrid entry points: they select either this mock behavior or the shipped production
auth adapter. A route location under `/api` does not by itself make behavior mock or production.
...
| Production Luas Go auth | `/api` | `AUTH_ADAPTER_ENABLED=true` plus adapter settings | Auth routes use the fixed production adapter; unrelated mock routes stay disabled. |
...
- Mock-only `src/app/api/**/route.ts` files call `guardMockBffRoute()` before reading request bodies
  ...
  only when it selects the mock BFF.
